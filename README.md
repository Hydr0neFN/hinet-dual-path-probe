...
### 2026-08-29 更新：故障換了位置

量測時間 2026-08-29 00:00 到 00:34，共 51 組配對樣本。浮動制對 Cloudflare 的專屬懲罰消失，中位數從 24 ms 降到 8 ms。但固定制同時變差，Cloudflare 中位數從 3 ms 升到 8 ms，兩條路徑延遲中位數打平；往 Tokyo relay 的延遲也兩條同時上升約 5 ms。兩條路徑的 jitter（mdev）更同時從 0.43 ms（固定）與 0.69 ms（浮動）升到 3.86 ms——差距縮小不等於變好，而是兩邊一起惡化。

逐段定位顯示，LAN 端的 gateway mdev 只有 0.062 ms，非常乾淨——問題不在家裡這一段。抖動從 BRAS 之後第一跳 168.95.94.134 開始，兩條 session 各測兩輪，四組數字彼此接近：

| 量測節點 (mdev) | 固定制 | 浮動制 |
| :--- | :--- | :--- |
| BRAS 後第一跳 168.95.94.134 | 3.64 / 4.05 ms | 3.90 / 3.73 ms |
