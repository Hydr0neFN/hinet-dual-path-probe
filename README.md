...
主探針每 45 秒取樣一次。玩家在遊戲裡感覺到的跳動發生在兩次取樣之間，所以中位數 / p95 回答不了「會不會跳」這個問題。

原本量遊戲路徑用的 UDP 方法**沒辦法拉高頻率**——relay 對回應做 rate limiting（token bucket），送再快回應數也卡在 9～13 個。所以改用 ICMP。

代價要講清楚：**ICMP 不是遊戲的 UDP 5-tuple，可能走到不同的 ECMP bucket**。真正符合 5-tuple 的 UDP 方法又跑不了高頻。這套工具沒辦法同時滿足兩個條件。

實測條件：`ping -i 0.05`（20 pps）、60 秒、**兩條路徑同時、同一個目標**（Tokyo SDR relay），而且是平靜時段、沒有劣化事件。兩條都 0% 掉包。

| | 中位數 | p95 | p99 | 最大 | 標準差 | 逐包抖動 | >60 ms 次數 |
|---|---|---|---|---|---|---|---|
| 固定制 | 33.7 | 35.3 | 52.6 | 59.6 | 2.68 | 1.02 | 0 |
| 浮動制 | 33.8 | 41.6 | 57.6 | 71.9 | 4.23 | 1.45 | 4 |

（單位都是 ms，「逐包抖動」是相鄰兩包 RTT 差的絕對值平均）

怎麼讀這張表：**中位數一模一樣，穩定度不一樣。** p95 差 6 ms、標準差高 58%、浮動制有 4 次衝破 60 ms 而固定制掛零。

幾分鐘前另一次 10 pps 的量測方向一致：`mdev` 2.70 vs 7.65、最大值 58 vs 86 ms。

這是目前唯一直接支持「固定制比較穩」的量測，而且**主探針的取樣率永遠看不到它**。
